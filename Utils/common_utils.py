...
# @Last modified time: 2018-03-11 00:02:22
...
# Time the give function
def time_it(func, *args, **kwargs):
	def eval_func():
		return func(*args, **kwargs)
	return timeit.timeit(eval_func, number = 1)

# Display a progress bar using the given specifications
...
# Apply the given function to each element in the given list and display the progress of this loop
def loop_display_progress(l, func, *func_args):
	...
	for i in range(0, len(l)):
		...
		# Add return value of this function execution to the result list
...
def return_loop_display_progress(l, func, *func_args):
	# Results for executing the function on each element in the list
	results = []
	# Track total runtime
	total_runtime = 0
	# Set the last progress display time to the current time
	...
	for i in range(0, len(l)):
		...
		start = time.clock()
		# Execute the function on the list element using the passed in arguments and add the result of this execution to the results list
		results.append(func(l[i], *func_args))
		# Finish function timing
