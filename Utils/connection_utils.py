...
# @Last modified time: 2018-03-11 00:41:14
...
def new_connection(host = os.environ.get("DBHOST"), dbname = os.environ.get("DBNAME"), user = os.environ.get("DBUSER"), password = os.environ.get("DBPASS"), port = os.environ.get("DBPORT"), primary = False, pass_to_function = True):
	def uses_db_dec(func):
		@wraps(func)
		def wrapper(*args, **kwargs):
			try:
				connection = psycopg2.connect("dbname='%s' user='%s' host='%s' password='%s' port=%s" % (dbname, user, host, password, port))
				print("\nConnection opened to [dbname:%s] on [%s:%s]" % (dbname, host, port))
				print("Logged in as [user:%s]\n" % user)
			except Exception as e:
				print("\nFailed to connect to the database.")
				print("Tried to connect to [dbname:%s] on [%s:%s] as [user:%s]" % (dbname, host, port, user))
				print("Make sure you have a .env file that that the DBNAME, DBUSER, DBHOST, DBPASS, and DBPORT keys are all correct.")
				return
...
def open_new_connection(host = os.environ.get("DBHOST"), dbname = os.environ.get("DBNAME"), user = os.environ.get("DBUSER"), password = os.environ.get("DBPASS"), port = os.environ.get("DBPORT")):
	try:
		connection = psycopg2.connect("dbname='%s' user='%s' host='%s' password='%s' port='%s'" % (dbname, user, host, password, port))
		print("\nConnection opened to [dbname:%s] on [%s:%s]" % (dbname, host, port))
		print("Logged in as [user:%s]\n" % user)
